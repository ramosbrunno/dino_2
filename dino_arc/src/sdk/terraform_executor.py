...
class TerraformExecutor:
    ...
    def _run_terraform_command(self, command: List[str], env_vars: Optional[Dict[str, str]] = None) -> subprocess.CompletedProcess:
        ...
        # Usar o caminho completo do terraform se necessário
        terraform_cmd = command[0]
        if terraform_cmd == "terraform":
            # Tentar encontrar terraform no PATH ou usar caminho padrão do Windows
            try:
                terraform_path = shutil.which("terraform")
                if terraform_path:
                    command[0] = terraform_path
                else:
                    # Caminho padrão do WinGet
                    winget_path = r"C:\Users\User\AppData\Local\Microsoft\WinGet\Packages\Hashicorp.Terraform_Microsoft.Winget.Source_8wekyb3d8bbwe\terraform.exe"
                    if os.path.exists(winget_path):
                        command[0] = winget_path
            except Exception:
                pass  # Usar terraform original se der erro
    ...
    def apply(self, variables: Optional[Dict[str, str]] = None, plan_file: str = "tfplan", auto_approve: bool = True) -> subprocess.CompletedProcess:
        ...
        # Remover arquivo de plano obsoleto se existir
        plan_path = self.working_dir / plan_file
        if plan_path.exists():
            try:
                plan_path.unlink()
                print(f"🗑️ Removido plano obsoleto: {plan_file}")
            except Exception as e:
                print(f"⚠️ Aviso: Não foi possível remover plano obsoleto: {e}")
        
        # Aplicar diretamente sem arquivo de plano
