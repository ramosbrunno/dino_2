"""
🦕 DINO SDK v1.1.6 - KeyVault Config Integrado
Configuração integrada com detecção robusta de ambiente Databricks
"""
...
class DinoKeyVaultConfig:
    """
    Configuração principal do Key Vault com detecção robusta de Databricks
    """
    ...
    def __init__(self, force_databricks: bool = False):
        ...
        print("🦕 DINO SDK v1.1.6 - KeyVault Config Integrado")
        print("=" * 55)
    ...
    def is_databricks_environment(self) -> bool:
        ...
        # Método 5: Verificar diretórios típicos do Databricks
        databricks_paths = [
            '/databricks',
            '/dbfs',
            '/local_disk0'
    ...
    def extract_context_inline(self) -> Dict[str, Any]:
        """
        Extrai contexto diretamente usando dbutils
        """
        try:
    ...
    def test_connection(self, force: bool = False) -> bool:
        """
        Testa conexão completa (extração + secrets)
        """
        try:
...
def dino_quick_setup(force: bool = False) -> DinoKeyVaultConfig:
    """
    Configuração rápida do KeyVault - função principal
    
    Args:
        force: Força execução mesmo se ambiente não for detectado
    
    Returns:
        Instância configurada do DinoKeyVaultConfig
    """
    config = DinoKeyVaultConfig(force_databricks=force)
    ...
    if config.test_connection(force=force):
        ...
        if not force:
            print("💡 Tente: dino_quick_setup(force=True) se estiver no Databricks")
        raise RuntimeError("❌ Falha na configuração inicial")

def dino_extract_context(use_cache: bool = True, force: bool = False) -> Dict[str, Any]:
    """
    Função para extrair apenas o contexto Databricks
...
def dino_simple_test():
    """
    Teste rápido para executar no Databricks
    """
    print("🦕 DINO SDK v1.1.6 - Teste Simples")
    print("=" * 40)
    
    # Teste 1: Verificar variáveis de ambiente
    print("\n1️⃣ Verificando ambiente...")
    env_vars = ['DATABRICKS_RUNTIME_VERSION', 'SPARK_HOME', 'DB_HOME']
    found_vars = []
    for var in env_vars:
        value = os.getenv(var)
        if value:
            found_vars.append(f"{var}={value[:30]}...")
            print(f"   ✅ {var}")
    
    if found_vars:
        print(f"✅ {len(found_vars)} variáveis Databricks encontradas")
    
    # Teste 2: Tentar setup
    print("\n2️⃣ Testando setup...")
    try:
