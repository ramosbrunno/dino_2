...
EXECUÇÃO:
- Em notebook Databricks: Execute direto (spark está disponível)
- Em ambiente standalone: A função get_spark_session() criará uma sessão
"""
...
def get_spark_session():
    """
    Obtém sessão Spark baseada no contexto de execução.
    """
    try:
        # Tenta usar spark global (contexto Databricks)
        return spark
    except NameError:
        # Cria nova sessão para ambiente standalone
        print("🔧 Criando nova sessão Spark...")
        return SparkSession.builder.appName("DINO SDK Ingestion").getOrCreate()
...
def exemplo_avancado():
    ...
    # Criar engine
    engine = IngestionEngine(spark_session)
...
def exemplo_validacao_schema():
    ...
    try:
        ...
        if not schema_manager.schema_exists(spark_session):
            print("📂 Schema não existe, criando...")
            result = schema_manager.create_schema(spark_session)
            if result['success']:
...
def exemplo_notebook_databricks():
    """
    Exemplo específico para uso em notebook Databricks.
    Copia este código diretamente para sua célula.
    """
    example_code = '''
# 🦕 DINO SDK - Uso em Notebook Databricks
from src.dino_sdk.ingestion_engine import ingest_csv_to_unity_catalog

# Ingestão simples de CSV
result = ingest_csv_to_unity_catalog(
    spark=spark,  # spark já disponível no Databricks
